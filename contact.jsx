                ...
                {!sent ? (
                  <form className="contact-form" onSubmit={onSubmit}>
                    <FormField label="Your name" required>
                      <input type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required placeholder="First and last" />
                    </FormField>
                    ...
                    <FormField label="Company + role">
                      <input type="text" value={form.company} onChange={e => setForm({ ...form, company: e.target.value })} placeholder="e.g. Acme, Head of Marketing" />
                    </FormField>
                      ...
                      <div className="radio-group">
                        {['Diagnostic · 2w', 'Project · 6–12w', 'Retainer · 3–6mo', 'Build · 4–8w', 'Not sure yet'].map(v => (
                          <label key={v} className={'radio-chip' + (form.budget === v ? ' is-active' : '')}>
                            <input type="radio" name="budget" value={v} checked={form.budget === v} onChange={e => setForm({ ...form, budget: e.target.value })} />
                            {v}
                          </label>
                        ))}
                      </div>
                    ...
                    <FormField label="What's the problem, in your own words" required help="One paragraph is fine. More is better. I read every one.">
                      <textarea
                        rows={6}
                        value={form.problem}
                      ...
                      <div className="radio-group">
                        {['Now', 'This quarter', 'Next quarter', 'Just exploring'].map(v => (
                          <label key={v} className={'radio-chip' + (form.timing === v ? ' is-active' : '')}>
                            <input type="radio" name="timing" value={v} checked={form.timing === v} onChange={e => setForm({ ...form, timing: e.target.value })} />
                            {v}
                          </label>
                        ))}
                      </div>
              ...
              <aside className="contact-aside">
                ...
                <div className="contact-card">
                  ...
                  <div className="contact-links">
                    ...
                    <a href="#" className="link-draw">Read.cv ↗</a>
                    <a href="#" className="link-draw">Are.na ↗</a>
                  </div>
                </div>
